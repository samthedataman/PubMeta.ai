...
def run_jobs():
    def join_dataframes_ranking(symptoms_df, triggers_df, comorbidities_df):
        ...
        symptoms_df = symptoms_df.pivot(
            index="conditions", columns="symptoms", values="rankings"
        ).fillna(0)
        triggers_df = triggers_df.pivot(
            index="conditions", columns="triggers", values="rankings"
        ).fillna(0)
        comorbidities_df = comorbidities_df.pivot(
            index="conditions", columns="comorbidities", values="rankings"
        ).fillna(0)
        joined_df = pd.concat(
            [symptoms_df, triggers_df, comorbidities_df], axis=1
        ).fillna(0)
        return joined_df
    ...
    FROM (
        ...
        FROM
            `airflow-test-371320.DEV.STUFF_THAT_WORKS_SYMPTOMS_DEV_FULL`
...
""",
    ...
    FROM (
        SELECT 
            conditions,
            cast(REGEXP_REPLACE(rankings, '#', '') as int) as rankings,
            cast(num_reports as int) as num_reports ,
            TimeScraped,
            DateScraped,
            replace(treatments,"|","") as treatments,
            TreatmentType,
            RANK() OVER (PARTITION BY conditions ORDER BY DateScraped DESC, TimeScraped DESC) as rank
        FROM 
