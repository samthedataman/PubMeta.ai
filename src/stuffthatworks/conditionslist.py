        ...
        if response.status_code == 200:
            ...
            for match in matches:
                if match.get("conditionState") == "public":
                    ...
                    pprint(match.get("conditionUrlId"),indent=True)
...
def get_public_conditions_treatments_new(treatments):
    base_url = "https://www.stuffthatworks.health/api/search?query="
    public_treatments = []

    total_conditions = len(treatments)

    for index, condition in enumerate(treatments):
        url = f"{base_url}{condition}"
        response = requests.get(url)
        ...
        if response.status_code == 200:
            ...
            public_treatments.append(df)

            percent_done = ((index + 1) / total_conditions) * 100
            print(percent_done)
            # Concatenate all dataframes
        result = pd.concat(public_treatments, ignore_index=True)
    
    return result

##conditions

treatments_df = pd.read_csv("/Users/samsavage/PythonProjects/PubMedGPT/stuffthatworks/treatments_conditions.csv")
# print(treatments_df)

treatments_list = treatments_df['treatments'].unique()

df = get_public_conditions_treatments_new(treatments_list)

df.to_csv("full_frame_treatments.csv")
